...
It is possible to render the diagrams to an SVG at build time.
This is useful for server-side rendering (SSR) and to display diagrams to users without JavaScript.
...
Then in your config, add the `output` option:
...
This documentation website uses the component mode, so a live example of an SVG rendered diagram cannot be shown.
